class Game_Character
    ...
    def can_move_from_coordinate?(start_x, start_y, dir, strict = false)
      ...
      when 1, 3   # Down diagonals
        ...
        x_diff = (dir == 1) ? @width * -1 : @width
  
        x_tile_offset = (dir == 1) ? 1 : -1
        y_tile_offset = -1
      ...
      when 7, 9   # Up diagonals
        ...
        (start_x...(start_x + @width)).each do |i|
          ...
          puts @height
          puts "Now add"
          puts start_y - (@height + 1)
          ((start_y - (@height + 1))..start_y).each do |j|
            puts "Start Y compared to j"
            puts start_y
            puts j
            if passable_y == true
              passable_y = false if !passable?(start_x + x_diff, j, dir - 3, strict)
            end
            if passable_x == true
              passable_x = false if !passable?(i + x_diff, j - 1 + 1, 8, strict)
            end
          end
        end
