...
class PokemonParty_Scene
  def pbStartScene(party, partner, starthelptext, annotations = nil, multiselect = false, can_access_storage = false)
    @sprites = {}
    @party = party
    @partner = partner
    ...
    if @partner
      @sprites["pokemon6"] = PokemonPartyPanel.new(@partner, 6, @viewport)
    else 
      @sprites["pokemon6"] = PokemonPartyBlankPanel.new(@partner, 6, @viewport)
    end
    @sprites["pokemon6"].text = annotations[6] if annotations
  ...
  def pbAnnotate(annot)
    ...
    @sprites["pokemon6"].text = (annot) ? annot[i] : nil
  end
  ...
  def pbSelect(item)
    ...
    @sprites["pokemon6"].selected = (6 == @activecmd)
  end
  ...
  def pbSwitchEnd(oldid, newid)
    ...
    (Settings::MAX_PARTY_SIZE+1).times do |i|
  ...
  def pbChangeSelection(key, currentsel)
    numsprites = Settings::MAX_PARTY_SIZE + ((@multiselect) ? 3 : 2)
    case key
    when Input::LEFT
      loop do
        currentsel -= 1
        break unless currentsel > 0 && currentsel < (Settings::MAX_PARTY_SIZE+1) && currentsel != (Settings::MAX_PARTY_SIZE) || !@party[currentsel]
      end
      if currentsel >= @party.length && currentsel < (Settings::MAX_PARTY_SIZE+1)
    ...
    when Input::RIGHT
      loop do
        currentsel += 1
        break unless currentsel < Settings::MAX_PARTY_SIZE+1 && currentsel != (Settings::MAX_PARTY_SIZE) || !@party[currentsel]
      end
    ...
    when Input::UP
      if currentsel >= Settings::MAX_PARTY_SIZE
        currentsel -= 1
        while currentsel > 0 && currentsel < Settings::MAX_PARTY_SIZE+1 && currentsel != (Settings::MAX_PARTY_SIZE) || !@party[currentsel]
          currentsel -= 1
        end
        currentsel = numsprites - 1 if currentsel < Settings::MAX_PARTY_SIZE+1 && currentsel >= @party.length+1
      else
        loop do
          currentsel -= 2
    ...
    when Input::DOWN
      if currentsel >= Settings::MAX_PARTY_SIZE - 1
        ...
        currentsel += 2
        currentsel = Settings::MAX_PARTY_SIZE if currentsel < Settings::MAX_PARTY_SIZE+1 && currentsel != (Settings::MAX_PARTY_SIZE) || !@party[currentsel]
      end
      if currentsel >= @party.length+1 && currentsel < Settings::MAX_PARTY_SIZE+1
        currentsel = Settings::MAX_PARTY_SIZE+1
  ...
  def pbHardRefresh
    ...
    lastselected = Settings::MAX_PARTY_SIZE+1 if lastselected < 0
    (Settings::MAX_PARTY_SIZE+1).times do |i|
      if @party[i] || i == 6
        @sprites["pokemon#{i}"] = PokemonPartyPanel.new(@party[i], i, @viewport)
      else
...
class PokemonPartyScreen
  ...
  def initialize(scene, party, partner)
    @scene = scene
    @party = party
    @partner = partner
  end
  ...
  def pbPokemonMultipleEntryScreenEx(ruleset)
    ...
    loop do
      ...
      if realorder.length == ruleset.number && addedEntry
        @scene.pbSelect(Settings::MAX_PARTY_SIZE+1)
      end
      @scene.pbSetHelpText(_INTL("Choose Pokémon and confirm."))
      pkmnid = @scene.pbChoosePokemon
      addedEntry = false
      if pkmnid == Settings::MAX_PARTY_SIZE+1   # Confirm was chosen
        ret = []
        realorder.each do |i|
          ret.push(@party[i])
        end
  ...
  def pbPokemonScreen
    ...
    @scene.pbStartScene(@party, @partner,
                        (@party.length > 1) ? _INTL("Choose a Pokémon.") : _INTL("Choose Pokémon or cancel."),
...
def pbChoosePokemon(variableNumber, nameVarNumber, ableProc = nil, allowIneligible = false)
  ...
  if chosen == 6
    pbSet(nameVarNumber, $player.partner.name)
  elsif chosen >= 0
